<?php
// Font Migration
require get_template_directory() . '/includes/fonts/custom-font-migration.php';
...
add_filter( 'jetpack_google_fonts_list', 'alicorn_filter_jetpack_google_fonts_list' );

$alicorn_enqueued_font_slugs = array();
 ...
 */
function collect_fonts_from_alicorn() {
	$fonts                  = array();
	...
	foreach ( $font_families as $font ) {
		...
		if ( array_key_exists( 'provider', $font ) && 'alicorn-fonts' === $font['provider'] ) {
			$fonts[] = $font;
		}
	}
...
function enqueue_global_styles_fonts() {
	...
	global $alicorn_enqueued_font_slugs;

	$font_slugs = array();
	$font_css   = '';

	if ( is_admin() ) {
		$font_families = collect_fonts_from_alicorn();
	...
	$alicorn_enqueued_font_slugs = $font_slugs;
	...
	wp_register_style( 'alicorn_font_faces', '' );
	wp_enqueue_style( 'alicorn_font_faces' );

	// Add the styles to the stylesheet.
	wp_add_inline_style( 'alicorn_font_faces', $font_css );
}
...
function enqueue_fse_font_styles( $fonts ) {
	$fonts    = collect_fonts_from_alicorn();
	$font_css = '';
...
function enqueue_block_fonts( $content, $parsed_block ) {
	...
	if ( ! is_admin() && isset( $parsed_block['attrs']['fontFamily'] ) ) {
		...
		if ( ! in_array( $font_slug, $alicorn_enqueued_font_slugs, true ) ) {
			$font_css = get_style_css( $font_slug );
			if ( $font_css ) {
				$alicorn_enqueued_font_slugs[] = $font_slug;
				wp_add_inline_style( 'alicorn_font_faces', $font_css );
			}
		}
	}
 ...
 */
function alicorn_filter_jetpack_google_fonts_list( $list_to_filter ) {
	return array();
}
